WLM 是 Redshift 用來**管理查詢資源分配**的機制 (traffic shaping)，核心概念是把不同類型的查詢放進不同的 queue，避免一個重型查詢拖垮整個 cluster
...
> if high_importance_flag:
        redshift_cur.execute("SET query_group TO 'bi_dag_long_queries';")
> ```
