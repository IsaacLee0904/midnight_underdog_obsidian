...
- How to register and notify consumers : The stakeholder list will be declared in data_keeper YAML files, but the mechanism for how they get notified requires further discussion. Three options under consideration :
	- **Option A — OM Follower** : This depends on how broadly we want to open up OpenMetadata. If we provision OM accounts for all stakeholders (including non-BI users), data_keeper can use the YAML as the source of truth and register each consumer as a Follower on the corresponding OM entity. When the entity changes, OM notifies all Followers automatically. Downside: requires creating and managing OM accounts for users outside the BI team.
